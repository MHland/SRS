...
class SRS:
    def __init__(self, n, bl, bu, p=3, sp=None, deps=12, delta=0.01, Vectorization=False, \
                 num=0, MAX=False, OptimalValue=None, ObjectiveLimit=None, eps=4, \
                 ShortLambda=0.02, LongLambda=0.2, InitialLt=3, Lt=2, ShowProgress=False):
        '''
    ...
    def __calculate_FE(self):
        if self.Vectorization:
            ...
            self.EachParFE = np.hstack(
                (self.EachParFE, self.EachPar[:, self.s:self.s + 1] * np.ones((self.n, self.MM))))
    ...
    def __select_points(self, Xp, Xb, x, j, p1, pp):
        ...
        ra = np.array([np.mod(j + 1, p1), np.mod(j, p1)])
            ...
            if Index > lt:
                ...
                if np.abs(np.log10(np.max([ineed, 10 ** (-self.eps - 1)]))) >= self.eps:
                    if self.show_progress:
                        max_arrow = 50
                        num_arrow1 = int(self.fe * max_arrow / self.num)  # show '>'
                        num_line1 = max_arrow - num_arrow1  # show '-'
                        percent1 = self.fe * 100.0 / self.num
                        sshow2 = np.abs(np.min(np.log10(Mbounds / M)))
                        num_arrow2 = int(sshow2 * max_arrow / self.deps)  # show '>'
                        num_line2 = max_arrow - num_arrow2  # show '-'
                        percent2 = sshow2 * 100.0 / self.deps
                        if percent1 > 100:
                            percent1 = 100
                        if percent2 > 100:
                            percent2 = 100
                        if percent1 > percent2:
                            process_bar = '[' + '>' * num_arrow1 + '-' * num_line1 + ']' + '%.2f' % percent1 + '%' + '\r'
                        else:
                            process_bar = '[' + '>' * num_arrow2 + '-' * num_line2 + ']' + '%.2f' % percent2 + '%' + '\r'
                        # print(process_bar, file=sys.stdout, flush=True)
                    ...
                    BestX = Xp1
                    maxpize = np.max(psize)
                    for i in range(p1):
                        x[:, i * maxpize:(i + 1) * maxpize] = Xp1[:, i:i + 1] * np.ones((1, maxpize))
                    ...
                    LL = np.zeros((self.n, maxpize))
                    for i in range(self.n):
                        LL[i, :] = [self.bl[i, 0] + k[i, 0] * j for j in range(maxpize)]
                    Index1 = 0
                    ...
                    for i in range(self.n):
                        for j in range(p1):
                            xneed = LL[Pi[i, j], 0:np.max(psize) - 1] + k[Pi[i, j], 0] * np.random.uniform(0, 1, size=(
                            1, psize[Pi[i, j], 0] - 1))
                            x[Pi[i, j], j * psize[Pi[i, j], 0] + 1:(j + 1) * psize[Pi[i, j], 0]] = xneed
                            x[Pi[i, j], j * psize[Pi[i, j], 0]] = x[Pi[i, j], 0] + k[Pi[i, j], 0] * (
                                        np.random.rand(1)[0] * 2 - 1)
                            if x[Pi[i, j], j * psize[Pi[i, j], 0]] < self.bl[Pi[i, j], 0]:
                                x[Pi[i, j], j * psize[Pi[i, j], 0]] = self.bl[Pi[i, j], 0] + k[Pi[i, j], 0] * (
                                np.random.rand(1)[0])
                            elif x[Pi[i, j], j * psize[Pi[i, j], 0]] > self.bu[Pi[i, j], 0]:
                                x[Pi[i, j], j * psize[Pi[i, j], 0]] = self.bu[Pi[i, j], 0] - k[Pi[i, j], 0] * (
                                np.random.rand(1)[0])
                        y = self.__calculate_function(x, func, *args)
                        Index1 = Index1 + 1
                        for j in range(p1):
                            ...
                            BestY[Index1, j] = nash
                            x[Pi[i, j], j * psize[Pi[i, j], 0]:(j + 1) * psize[Pi[i, j], 0]] = x[Pi[i, j], j * psize[
                                Pi[i, j], 0] + index] * np.ones((1, maxpize))
                    ...
                    self.bu[heihei1[:, 0], 0] = np.min(
                        np.append(nx1[heihei1[:, 0]] + k[heihei1[:, 0]], BU[heihei1[:, 0]], axis=1), axis=1)
                    self.bl[heihei2[:, 0], 0] = np.max(
                        np.append(nx2[heihei2[:, 0]] - k[heihei2[:, 0]], BL[heihei2[:, 0]], axis=1), axis=1)
                    self.__calculate_FE()
                    self.s = self.s + 1
        T2 = time.time()
        Result = []
    ...
    def UrsemWaves(x):
        x1 = x[:, 0:1]
        x2 = x[:, 1:2]
        y = -(0.3 * x1) ** 3 + (x2 ** 2 - 4.5 * x2 ** 2) * x1 * x2 + 4.7 * np.cos(3 * x1 - x2 ** 2 * (2 + x1)) * np.sin(
            2.5 * np.pi * x1)
        return y
...
if __name__ == '__main__':
    ...
    bl = [-5 for i in range(n)]
    bu = [10 for i in range(n)]
    srs = SRS(n, bl, bu, p=10, sp=3, delta=0.35, deps=5, MAX=False, Vectorization=True, num=5000, OptimalValue=0,
              ObjectiveLimit=10 ** -20, ShowProgress=True)
    srs.run(SRS.Zakharov)
    srs.type_result()
    print('*********************************************')